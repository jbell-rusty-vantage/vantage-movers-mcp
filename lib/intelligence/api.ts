...
export type IntelligenceApi = (
  ...
  action: "context" | "read" | "submit" | "submission",
  body?: unknown,
) => Promise<unknown>;
...
export function createIntelligenceApi(
  ...
  return async (credentials, action, body) => {
    try {
      ...
      if (!configured)
        throw new IntelligenceError("INTELLIGENCE_UNAVAILABLE", 503);
      ...
      const response = await (options.fetchImpl ?? fetch)(
        new URL(
          `/api/v1/internal/sales-intelligence/runs/${credentials.claims.run_id}/${action}`,
          base,
        ),
        {
